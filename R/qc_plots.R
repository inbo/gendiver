...
#' @param all_wells (default=FALSE) Plot all wells, regardless if data is present or not. If turned off (`mark_all=FALSE`, default), only plot wells with data above the omit cutoff.
#'
#' @returns ggplot2::geom_tile()
#' @export
#'
...
qcplot.plate_heatmap_readcount = function(
  ...
  mark_pal = colorspace::qualitative_hcl(length(unique(sample_metadata$ANALYSE_TYPE)))
  names(mark_pal) = unique(sample_metadata$ANALYSE_TYPE)
